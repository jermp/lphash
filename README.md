...
to show the usage of the driver program (reported below for convenience).
    ...
    [-d tmp_dirname]
        Temporary directory used for construction in external memory (default is current directory '.').
    ...
    [--in-memory]
        Do not use external memory during the construction of the internal MPHFs (disabled by default).
