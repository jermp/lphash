...
inline u_int64_t printPt(pthread_t pt) {
	...
	u_int64_t res = 0;
	for (size_t i = 0; i < sizeof(pt); i++) { res += (unsigned)(ptc[i]); }
	return res;
}
...
class bfile_iterator : public std::iterator<std::forward_iterator_tag, basetype> {
	...
	bfile_iterator(FILE* is) : _is(is), _pos(0), _inbuff(0), _cptread(0) {
		...
		_buffsize = 10000;
		_buffer = (basetype*)malloc(_buffsize * sizeof(basetype));
		if (fseek(_is, 0, SEEK_SET)) fprintf(stderr, "[Error] fseek failed\n");
		advance();
	}
	...
	void advance() {
		...
		if (_cptread >= _inbuff) {
			...
			// fprintf("read %i new elem last %llu  %p\n",res,_buffer[res-1],_is);
			_inbuff = res;
			_cptread = 0;
...
class file_binary {
	...
	file_binary(const char* filename) {
		...
		if (!_is) { throw std::invalid_argument("Error opening " + std::string(filename)); }
	}
...
class Progress {
	...
	void inc(uint64_t ntasks_done) {
		...
		while (partial >= steps) {
			if (timer_mode) {
				...
				fprintf(stderr, "%c[%s]  %-5.3g%%   elapsed: %3i min %-2.0f sec   remaining: %3i min %-2.0f sec", 13, message.c_str(), 100 * (double)done / todo, min_e, elapsed, min_r, rem);
	...
	void inc(uint64_t ntasks_done, int tid) // threads collaborate to this same progress bar
	{
		...
		while (partial_threaded[tid] >= steps) {
			if (timer_mode) {
				...
				fprintf(stderr, "%c[%s]  %-5.3g%%   elapsed: %3i min %-2.0f sec   remaining: %3i min %-2.0f sec", 13, message.c_str(), 100 * (double)total_done / todo, min_e, elapsed, min_r, rem);
			} else {
				fprintf(stderr, "-");
				fflush(stderr);
			}
	...
	void set(uint64_t ntasks_done) {
		if (ntasks_done > done) inc(ntasks_done - done);
	}
...
class HashFunctors {
	...
	hash_set_t operator()(const Item& key) {
		...
		for (size_t ii = 0; ii < 10; ii++) { hset[ii] = hash64(key, _seed_tab[ii]); }
		return hset;
	}
...
class XorshiftHashFunctors {
	...
	hash_set_t operator()(const Item& key) {
		...
		for (size_t ii = 2; ii < 10 /* it's much better have a constant here, for inlining; this loop is super performance critical*/; ii++) { hset[ii] = next(s); }
...
class bitVector {
	...
	~bitVector() {
		if (_bitArray != nullptr) free(_bitArray);
	}
	...
	// Move constructor
	bitVector(bitVector&& r) : _bitArray(nullptr), _size(0) { *this = std::move(r); }
	...
	void clearCollisions(uint64_t start, size_t size, bitVector* cc) {
		...
		for (uint64_t ii = 0; ii < (size / 64ULL); ii++) { _bitArray[ids + ii] = _bitArray[ids + ii] & (~(cc->get64(ii))); }
	...
	// for debug purposes
	void print() const {
		fprintf(stderr, "bit array of size %lli: \n", (long long int)_size);
		for (uint64_t ii = 0; ii < _size; ii++) {
			if (ii % 10 == 0) fprintf(stderr, " (%llu) ", (long long unsigned int)ii);
			int val = (_bitArray[ii >> 6] >> (ii & 63)) & 1;
			fprintf(stderr, "%i", val);
		}
		fprintf(stderr, "\n");

		fprintf(stderr, "rank array : size %lu \n", _ranks.size());
		for (uint64_t ii = 0; ii < _ranks.size(); ii++) { fprintf(stderr, "%llu :  %lli,  ", (long long unsigned int)ii, (long long int)_ranks[ii]); }
		fprintf(stderr, "\n");
	}
	...
	uint64_t build_ranks(uint64_t offset = 0) {
		...
		for (size_t ii = 0; ii < _nchar; ii++) {
			if (((ii * 64) % _nb_bits_per_rank_sample) == 0) { _ranks.push_back(curent_rank); }
	...
	uint64_t rank(uint64_t pos) const {
		...
		for (uint64_t w = block * _nb_bits_per_rank_sample / 64; w < word_idx; ++w) { r += popcount_64(_bitArray[w]); }
...
class mphf {
	...
	mphf(size_t n, Range const& input_range, int num_thread = 1, double gamma = 2.0, bool writeEach = true, bool progress = true, float perc_elem_loaded = 0.03)
		...
		if (_percent_elem_loaded_for_fastMode > 0.0) _fastmode = true;
		...
		if (_withprogress) {
			...
			fprintf(
				stderr, "for info, total work write each  : %.3f    total work inram from level %i : %.3f  total work raw : %.3f \n", total_writeEach, _fastModeLevel, total_fastmode_ram, total_raw);
		...
		if (_withprogress) _progressBar.finish_threaded();
	...
	uint64_t totalBitSize() {
		...
		for (int ii = 0; ii < _nb_levels; ii++) { totalsizeBitset += _levels[ii].bitset.bitSize(); }
		...
		fprintf(
			stderr, "Last level hash  %12lu  bits (%.2f %%) (nb in last level hash %lu)\n", _final_hash.size() * 42 * 8, 100 * (float)(_final_hash.size() * 42 * 8) / totalsize, _final_hash.size());
		return totalsize;
	}
	...
	void pthread_processLevel(std::vector<elem_t>& buffer, std::shared_ptr<Iterator> shared_it, std::shared_ptr<Iterator> until_p, int i) {
		...
		for (bool isRunning = true; isRunning;) {
			...
			//	fprintf(stderr, "filling input  buff \n");

			for (uint64_t ii = 0; ii < inbuff; ii++) {
				elem_t val = buffer[ii];
				// fprintf(stderr, "processing %llu  level %i\n",val, i);
	...
	void save(std::ostream& os) const {
		...
		for (int ii = 0; ii < _nb_levels; ii++) { _levels[ii].bitset.save(os); }
	...
	void setup() {
		...
		// fprintf(stderr, "pt self %llu  pid %i \n",printPt(pthread_self()),_pid);

		_cptTotalProcessed = 0;

		if (_fastmode) { setLevelFastmode.resize(_percent_elem_loaded_for_fastMode * (double)_nelem); }

		bufferperThread.resize(_num_thread);
		if (_writeEachLevel) {
			for (int ii = 0; ii < _num_thread; ii++) { bufferperThread[ii].resize(NBBUFF); }
		}

		_proba_collision = 1.0 - pow(((_gamma * (double)_nelem - 1) / (_gamma * (double)_nelem)), _nelem - 1);

		// double sum_geom =_gamma * ( 1.0 +  _proba_collision / (1.0 - _proba_collision));
		// fprintf(stderr, "proba collision %f  sum_geom  %f   \n",_proba_collision,sum_geom);
		...
		for (int ii = 0; ii < _nb_levels; ii++) {
			...
			// fprintf("build level %i bit array : start %12llu, size %12llu  ",ii,_levels[ii]->idx_begin,_levels[ii]->hash_domain );
			// fprintf(" expected elems : %.2f %% total \n",100.0*pow(_proba_collision,ii));
		}

		for (int ii = 0; ii < _nb_levels; ii++) {
			if (pow(_proba_collision, ii) < _percent_elem_loaded_for_fastMode) {
				_fastModeLevel = ii;
				// fprintf("fast mode level :  %i \n",ii);
				break;
			}
		}
	}
	...
	void insertIntoLevel(uint64_t level_hash, int i) {
		...
		if (_levels[i].bitset.atomic_test_and_set(hashl)) { _tempBitset->atomic_test_and_set(hashl); }
	}
	...
	void processLevel(Range const& input_range, int i) {
		...
		// fprintf("---process level %i   wr %i fast %i ---\n",i,_writeEachLevel,_fastmode);
		...
		if (_writeEachLevel && (i > 1)) {
			...
			for (int ii = 0; ii < _num_thread; ii++) pthread_create(&tab_threads[ii], NULL, thread_processLevel<elem_t, Hasher_t, Range, disklevel_it_type>, &t_arg); //&t_arg[ii]

			// must join here before the block is closed and file_binary is destroyed (and closes the file)
			for (int ii = 0; ii < _num_thread; ii++) { pthread_join(tab_threads[ii], NULL); }
			...
			if (_fastmode && i >= (_fastModeLevel + 1)) {
				...
				for (int ii = 0; ii < _num_thread; ii++) pthread_create(&tab_threads[ii], NULL, thread_processLevel<elem_t, Hasher_t, Range, fastmode_it_type>, &t_arg); //&t_arg[ii]

			} else {
				for (int ii = 0; ii < _num_thread; ii++) pthread_create(&tab_threads[ii], NULL, thread_processLevel<elem_t, Hasher_t, Range, decltype(input_range.begin())>, &t_arg); //&t_arg[ii]
			}
			// joining
			for (int ii = 0; ii < _num_thread; ii++) { pthread_join(tab_threads[ii], NULL); }
		}
