...
class unpartitioned : public interface {
public:
    unpartitioned();
    ...
    std::vector<uint64_t> operator()(const char* contig, std::size_t length, bool streaming) const;

    template <typename MinimizerHasher = pthash::murmurhash2_64>
    std::vector<uint64_t> operator()(std::string const& contig, bool streaming) const;

    template <typename Visitor>
    void visit(Visitor& visitor);

    friend std::ostream& operator<<(std::ostream& os, const unpartitioned& obj);
...
std::vector<uint64_t> unpartitioned::operator()(const char* contig, std::size_t length,
...
template <typename MinimizerHasher>
std::vector<uint64_t> unpartitioned::operator()(std::string const& contig, bool streaming) const {
    return operator()<MinimizerHasher>(contig.c_str(), contig.length(), streaming);
}

template <typename Visitor>
void unpartitioned::visit(Visitor& visitor) {
    visitor.visit(k);
    visitor.visit(m);
