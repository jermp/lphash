...
void unpartitioned::build(configuration const& config, std::ostream& res_strm) {
...
void unpartitioned::build_size_index(external_memory_vector<mm_triplet_t>::const_iterator& mm_itr,
                                [[maybe_unused]] std::size_t number_of_distinct_minimizers,
                                uint64_t size_sum) {
    assert(distinct_minimizers == number_of_distinct_minimizers);
...
void unpartitioned::print_statistics() const noexcept {
    auto mm_mphf_size_bits = minimizer_order.num_bits();
    auto positions_size_bits = positions.num_bits();
    auto sizes_size_bits = sizes.num_bits();
    auto kmer_mphf_size_bits = fallback_kmer_order.num_bits();
...
std::ostream& operator<<(std::ostream& out, unpartitioned const& hf) {
