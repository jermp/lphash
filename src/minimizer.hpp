...
[[nodiscard]] uint64_t from_string(
	char const* contig, std::size_t contig_size, uint32_t k, uint32_t m, uint64_t seed, bool canonical_m_mers, uint64_t& mm_count, lphash::sorted_external_vector<mm_record_t>& accumulator) {
		...
		if (c < 4) [[likely]] {
				...
				if (nbases_since_last_break >= m) {
					...
					if (nbases_since_last_break == k + 1)
						[[unlikely]] { // have seen the first window after a break, time to search for the minimum note that the current m-mer is checked by the next if
							min_pos = p1 = 0;
...
template <typename MinimizerHasher>
void get_colliding_kmers(char const* contig,
						 ...
						 sorted_external_vector<uint64_t>::const_iterator& stop,
						 uint64_t& mm_count,
					...
					if (nbases_since_last_break >= k + 1) [[likely]] { // time to update the minimum, if necessary
							...
							if (((mm_buf_pos) % mm_buffer.size()) == min_pos || current.itself < mm_buffer[min_pos].itself) { // update min
								...
								if (((mm_buf_pos) % mm_buffer.size()) == min_pos) {
									...
									min_pos = mm_buf_pos; // new minimum, actual update is outside if
								}
								sks = 0;
							}
							++sks;
...
std::pair<sorted_external_vector<mm_triplet_t>, sorted_external_vector<uint64_t>> classify(sorted_external_vector<mm_record_t>& minimizers, uint8_t max_memory, std::string tmp_dirname) {
	...
	sorted_external_vector<mm_triplet_t> unique_minimizers(
		unique_minimizer_mm_size_estimate, []([[maybe_unused]] mm_triplet_t const& a, [[maybe_unused]] mm_triplet_t const& b) { return false; }, tmp_dirname, get_group_id());
	sorted_external_vector<uint64_t> colliding_minimizer_ids(
		colliding_mm_size_estimate, [](uint64_t a, uint64_t b) { return a < b; }, tmp_dirname, get_group_id());
