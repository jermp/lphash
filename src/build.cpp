...
int main(int argc, char* argv[]) {
	...
	if (parser.parsed("seed"))
		mm_seed = parser.get<uint64_t>("seed");
	else
		mm_seed = 42;
	if (parser.parsed("threads"))
		nthreads = parser.get<uint32_t>("threads");
	else
		nthreads = 1;
	...
	if (parser.parsed("c"))
		...
		c = constants::c;
	if (parser.parsed("canonical_parsing"))
		canonical = parser.get<bool>("canonical_parsing");
	else
		canonical = false;
	...
	sorted_external_vector<mm_record_t> all_minimizers(
		uint64_t(max_memory) * essentials::GB, [](mm_record_t const& a, mm_record_t const& b) { return a.itself < b.itself; }, tmp_dirname, get_group_id());
	fp = NULL;
	...
	std::cerr << "Part 3: build inverted index\n";
	{
		sorted_external_vector<mm_triplet_t> mm_sorted_by_mphf(
			uint64_t(max_memory) * essentials::GB, [](mm_triplet_t const& a, mm_triplet_t const& b) { return a.itself < b.itself; }, tmp_dirname, get_group_id());
	...
	std::cout << input_filename << "," << static_cast<uint32_t>(k) << "," << static_cast<uint32_t>(m) << "," << static_cast<double>(total_colliding_minimizers) / total_distinct_minimizers << ","
			  << 2.0 / ((k - m + 1) + 1) << "," << static_cast<double>(total_minimizers) / total_kmers << "," << static_cast<double>(total_contigs) / total_kmers << ","
