...
quartet_wtree_builder::quartet_wtree_builder(std::size_t size) : m_root_size(0), m_left_right_size(0), m_max_none_size(0)
{
    root.reserve(size);
...
MinimizerType quartet_wtree::operator[](uint64_t idx) const
{
    ...
    return static_cast<MinimizerType>((static_cast<uint64_t>(msb) << 1) | static_cast<uint64_t>(lsb));
}
...
std::pair<MinimizerType, std::size_t> quartet_wtree::rank_of(std::size_t idx) const
{
    ...
    toRet.first = static_cast<MinimizerType>((static_cast<uint64_t>(msb) << 1) | static_cast<uint64_t>(lsb));
    return toRet;
}
