...
{
    // if (m_tmp_dirname != "") essentials::create_directory(m_tmp_dirname);
...
std::string sorted_external_vector<T>::get_tmp_output_filename(uint64_t id) const 
{
    ...
    if (m_tmp_dirname != "") filename << m_tmp_dirname << "/";
...
sorted_external_vector<T>::const_iterator::const_iterator() 
    : v(nullptr), heap_idx_comparator([]([[maybe_unused]] uint32_t i, [[maybe_unused]] uint32_t j){return false;}) {}
