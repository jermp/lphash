...
public:
  ...
  inline uint64_t rank(uint64_t pos) const {
    ...
    if (sub_left) {
      r += pthash::util::popcount(m_bits[sub_block] << (64 - sub_left));
    }
    return r;
  }
  ...
  inline uint64_t select(uint64_t n) const {
    ...
    uint64_t sub_block_offset =
        uleq_step_9(sub_ranks, rank_in_block_parallel) * ones_step_9 >> 54 &
        0x7;
    ...
    return word_offset * 64 +
           pthash::util::select_in_word(m_bits[word_offset], n - cur_rank);
  }
  ...
  uint64_t bytes() const {
    return pthash::bit_vector::bytes() +
           essentials::vec_bytes(m_block_rank_pairs) +
