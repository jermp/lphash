...
struct ef_sequence {
    ...
    void encode(ForwardIterator begin, uint64_t n, uint64_t u) {
        ...
        // n += 1; // work-around, since the older version (ef_sequence_inmemory) used to add a 0 at the beginning
        ...
        // if (l) cv_builder_low_bits.push_back(0); // we insert the dummy 0
        // bvb_high_bits.set(0, 1); // same, for the high bits
        // n -= 1; // we restore the true n
