...
template <typename MinimizerHasher>
std::vector<uint64_t> mphf_alt::operator()(std::string const& contig, bool streaming) const {
...
template <typename MinimizerHasher>
uint64_t mphf_alt::barebone_streaming_query(const char* contig, std::size_t length) const {
    using namespace minimizer;
    uint64_t res = 0;
    if (length < k) return res;
    uint64_t shift = 2 * (m - 1);
...
template <typename MinimizerHasher>
uint64_t mphf_alt::barebone_dumb_query(const char* contig, std::size_t length) const {
    uint64_t res = 0;
    for (std::size_t i = 0; i < length - k + 1; ++i) {
        auto kmer = debug::string_to_integer_no_reverse(&contig[i], k);
        debug::triplet_t triplet =
