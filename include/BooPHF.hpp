...
////////////////////////////////////////////////////////////////
	...
	public:
		...
		void print() const
		{
			...
			{
				if(ii%10==0)
					fprintf(stderr, " (%llu) ",(long long unsigned int)ii);
			...
			{
				fprintf(stderr, "%llu :  %lli,  ",(long long unsigned int)ii,(long long int)_ranks[ii]);
			}
			fprintf(stderr, "\n");
		}
	...
	public:
		...
		_gamma(gamma), _hash_domain(size_t(ceil(double(n) * gamma))), _nelem(n), _num_thread(num_thread), _percent_elem_loaded_for_fastMode (perc_elem_loaded), _withprogress(progress)
		{
			...
			if(_withprogress)
			{
				...
				fprintf(stderr, "for info, total work write each  : %.3f    total work inram from level %i : %.3f  total work raw : %.3f \n",total_writeEach,_fastModeLevel,total_fastmode_ram,total_raw);
